...
int main (void)
{
	...
	//Serial  UART 1
	_RP15R=0b00011;//U1TX 00011 RPn tied to UART1 Transmit pin 18 RP9
	RPINR18bits.U1RXR=0xE;//UART1 Receive U1RX RPINR18 U1RXR<4:0> pin 17 RP8
...
//	00000 = Input tied to RP0
	...
	/* Initialisation du timer 3 et ouput compare   
	http://ww1.microchip.com/downloads/en/DeviceDoc/70209A.pdf*/
	_RP4R=0b10010;   //Ça ce pour le Ouput compare module.
	...
	OC1CONbits.OCM = 0b110; // Enable Output Compare Module

	_RP5R=0b10011;   //Ça ce pour le Ouput compare module.
	...
	IPC1bits.OC2IP = 0x02; // Set Output Compare 1 Interrupt Priority Level
	OC2R = 100; // Load the Compare Register Value
	OC2CONbits.OCM = 0b110; // Enable Output Compare Module

	_RP12R=0b10100;   //Ça ce pour le Ouput compare module.
	...
	IPC6bits.OC3IP = 0x03; // Set Output Compare 1 Interrupt Priority Level
	OC3R = 100; // Load the Compare Register Value
	OC3CONbits.OCM = 0b110; // Enable Output Compare Module

	_RP13R=0b10101;   //Ça ce pour le Ouput compare module.
	...
	IPC6bits.OC4IP = 0x03; // Set Output Compare 1 Interrupt Priority Level
	OC4R = 100; // Load the Compare Register Value
	OC4CONbits.OCM = 0b110; // Enable Output Compare Module
	...
	PR3 = 4000; // Load the period value
	IPC2bits.T3IP = 0x03; // Set Timer 2 Interrupt Priority Level
...
{ 
if(read_adc1>2100) OC1RS = (2*read_adc1)-4000; // Write Duty Cycle value for next PWM cycle
if((read_adc1<=2100)&(read_adc1>=1900)) {OC1RS =0;OC2RS =0;} 
if(read_adc1<1900) OC2RS = (-2*read_adc1)+4000; // Write Duty Cycle value for next PWM cycle
if(read_adc2>2100) OC3RS = (2*read_adc2)-4000; // Write Duty Cycle value for next PWM cycle
if((read_adc2<=2100)&(read_adc2>=1900)) {OC3RS =0;OC4RS =0;} 
if(read_adc2<1900) OC4RS = (-2*read_adc2)+4000; // Write Duty Cycle value for next PWM cycle
} 
...
void __attribute__((interrupt, no_auto_psv)) _U1RXInterrupt(void)
 {
       _U1RXIF=0;                    // Clear UART RX Interrupt Flag.
		...
		{
			data1=ReadUART1_mod();
			if (data1==(unsigned int)0x00fd){prochendata=0; return;}
			if (data1==(unsigned int)0x00fe){prochendata=0; return;}
			prochendata=2;
			return;
		}	
		if (prochendata == 2)
		{
			data2=ReadUART1_mod();
			if (data2==(unsigned int)0x00fd){prochendata=0; return;}
			if (data2==(unsigned int)0x00fe){prochendata=0; return;}
			prochendata=0;
			dataready1=1;
		}	
		if (prochendata == 3)
		{
			data3=ReadUART1_mod();
			if (data3==(unsigned int)0x00fd){prochendata=0; return;}
			if (data3==(unsigned int)0x00fe){prochendata=0; return;}
			prochendata=4;
			return;
		}	
		if (prochendata == 4)
